...
sedlist <- list(
  ...
  EXT_BLANK = c('410','411','422','423'),
  LIB_BLANK = c('Lib_blank_1(1-12)','Lib_blank_2(1-12)','Lib_blank_1(13-24)','Lib_blank_2(13-24)')
)
...
p1 <- ggplot(data.perc, aes(fill=Var1, y=value, x=factor(Var2, level = level_order))) +
  ...
  theme(legend.position = "right",
        ...
        axis.text.x = element_text(size=20, angle = 90, vjust=0.4, hjust=1, color = "black"),
...
p3 <- ggplot(ratio, aes(x=factor(name, level = level_order), y=ratio, shape=Extraction, fill=Buffer, group=name)) +
  ...
  scale_fill_manual(
    ...
    guide = guide_legend(override.aes = list(shape = 21))) +
  theme(axis.text.x = element_text(size=20, angle = 90, vjust=0.4, hjust=1, color = "black"),
        axis.text.y = element_text(size = 20, color = "black"),
        axis.title = element_text(face="bold", size = 22, color = "black"),
        ...
        legend.key = element_rect(colour = NA, fill = NA),
        legend.key.size = unit(0.5, "cm"),
        strip.background = element_blank(), strip.text = element_text(size = 18, margin = margin(2,0,2,0))) +
  labs(x ="Sample", y = "Molarity ratio [157-500 bp] / [25-157 bp]") +
  facet_grid(. ~ factor(Sediment, level = sedlevel), scales = "free", space = "free")
...
figure <- ggarrange(p1, p2, p3,
                    ...
                    ncol = 1, nrow = 3, common.legend = FALSE, legend="right")
figure