...
ggplot(df, aes(x=Buffer, y=Percent, group = Sediment_ID, color = Sediment_ID)) + 
...
p1.1 <- p1 + geom_text(data = panel_text, aes(x = 1.5, y = 0.7, label = label), vjust = 1.5, hjust = 0.5, inherit.aes = FALSE)
...
ggplot(df, aes(x=Buffer, y=Absolute, group = Sediment_ID, color = Sediment_ID)) + 
  geom_smooth(method = lm, se = FALSE) +
  geom_point()+
  facet_grid( ~ Range) +
  theme_bw()
...
### one out of six ranges show significant difference between the two buffers in terms of absoulte molarity of 
### different ranges
...
p2 <- ggplot(df, aes(x=Buffer, y=Absolute, group = Sediment_ID, color = Sediment_ID)) + 
  ...
  geom_point() +
  facet_grid( ~ Range) +
  theme_bw() + ylim(0,40) + ylab("Absolute molarity [nM]") + ggtitle("Absolute molarity across fragment ranges")
...
p2.2 <- p2 + geom_text(data = panel_text, aes(x = 1.5, y = 40, label = label), vjust = 1.5, hjust = 0.5, inherit.aes = FALSE)

p2.2

### Merge both plots

figure <- ggarrange(p1.1, p2.2,
                    labels = c("A", "B"),
